# Jamie Cheng — Compact Profile (for token-efficient evaluations)

> This file is a condensed reference for quick go/pass checks.
> For full details, read `preferences.md`, `h1b_verified.md`, and `content_library.md`.

## Identity
- MS Applied Org Psych, USC (3.95) | BA Wesleyan (4.00)
- ~3 years: OD consulting, program management, HRBP, HR internships
- Requires H1B sponsorship or cap-exempt employer
- Location: Portland OR | Open to: Remote US, Seattle

## Hard Rejects (instant PASS)
- "No sponsorship" / "must be authorized without sponsorship"
- Senior / Director / VP / Principal / C-level (exception: "Senior Associate" at consulting firms)
- Pure Sales, Pure SWE, Senior HRBP, Instructional Designer (80%+ content), Technical PM (PMP)
- Pure recruiting/TA roles

## Role Priorities
1. **P1** People/HR Consulting (OD Consultant, Change Mgmt Consultant, Talent Dev Consultant)
2. **P2** People Program Management (People/HR/Talent Program Manager)
3. **P3** L&D Mgmt/Ops/Strategy (L&D Manager, Learning Ops, Training Manager, Talent Dev Specialist)
4. **P4** Junior HRBP (Associate HRBP, People Partner junior)
5. **P5** OD/Employee Experience (OD Specialist, EX Specialist, Change Mgmt Specialist)

## Self-Assessment (ground truth for match %)
| Level | Domains |
|-------|---------|
| ★★★ Strong | Program management, L&D ops/vendor mgmt, Employee experience, Data analytics |
| ★★☆ Developing | OD consulting, L&D strategy, HRBP (strategic), Change mgmt frameworks |
| ★☆☆ Basic | Instructional design, HR operations, Recruiting, Comp & total rewards |

**Auto-PASS if JD requires ★☆☆ skills as primary** (payroll, HRIS admin, benefits, ER casework, recruiting as main function). Override only if cap-exempt + perfect title + Portland + personal connection.

## Location & Remote Bar
- Portland/Seattle: 60-70% JD match OK
- Remote: 80%+ match PLUS additional advantage (cap-exempt, green-light company, P1 consulting, network)

## Fit Score Formula (v3.4.1)
Base: ⭐⭐⭐ Excellent = 82 | ⭐⭐ Good = 70 | ⭐ Near-miss = 58
Modifiers: Cap-exempt +10 | H1B confirmed +5 | Portland/Seattle +5 | P1/P2 +3 | Remote-only -10 | 3-4yr req -5
Cap 100, floor 0.

## H1B Quick Reference
**Confirmed sponsors:** Google, Meta, Amazon, Microsoft, Apple, Stripe, Spotify, Netflix, Disney, EA, Epic, Riot, HubSpot, T-Mobile, Salesforce, Adobe, Intel, Nike, Mercer, Aon, Korn Ferry, WTW (senior only), Accenture, McKinsey, Axon, dbt Labs, Plaid, BCG
**Cap-exempt:** PSU, U of Portland, Reed, LC, OSU, UO, UW, OHSU, PeaceHealth, Providence, Legacy, Kaiser NW, Seattle Children's, Fred Hutch
**No sponsor:** Deloitte, PCC, Customer.io, DriveWealth, Boeing
**Unknown:** Columbia Sportswear, Daimler Trucks NA, Nintendo of America, Valve, Cambia Health

## Green-Light Companies
Mercer, Aon, Korn Ferry, Deloitte*, Accenture, WTW, McKinsey, EY-Parthenon, AuditBoard, Duolingo, Form Energy, Stryker, Marsh McLennan, Starbucks, Amazon, Pathrise, Forage, Tandem HR, universities, hospitals, foundations
(*Deloitte = green-light but confirmed no H1B)
