...
SHOT     = ROLE_DIR + r"\screenshots\v14"
...
def fill_date_navigate(page, field_id):
    """
    v14: Navigate the date picker calendar to Sep 2026.
    Key insight from v12/v13:
    - Keyboard typing sets DOM .value but NOT React state (controlled component)
    - Calendar navigates correctly to Sep 2026
    - Day 1 cell has class "picker-cell-start" which may prevent React selection event
    New strategy:
    - Navigate to Sep 2026 (known-working)
    - Click day 2 FIRST (plain ocpicker cell, no picker-cell-start) → verify it commits
    - Then click day 1 to correct to Sep 1
    - Also try tap() on the TD (touch event path — completely separate from mouse/pointer)
    - Also try clicking via Patchright after focus is set on the day number span inside TD
    """
    if not alive(page): return False
    log(f"  [date] v14 Opening date picker...")
    ...
    try:
        ...
        def reopen_to_sep():
            """Reopen calendar and navigate to Sep if needed."""
            n = cal_cells_visible()
            if n == 0:
            ...
            if "sep" not in hdr:
                ...
                log(f"  [date] re-nav header: '{get_header()}'")

        log(f"  [date] Calendar cells after nav: {cal_cells_visible()}")

        # ── Approach 1: Click day 2 (no picker-cell-start), then day 1 ────────
        # Day 1 has class "picker-cell-start" which might suppress React's onClick.
        # Day 2 is a plain cell. If it commits, then we know the mechanism works,
        # and we click day 1 to get the right date.
        log(f"  [date] [1] Click day 2 first (avoids picker-cell-start), then day 1...")
        reopen_to_sep()
        ...
        log(f"  [date] [1] Day 2 coord: {day2}")
        if day2:
            page.mouse.click(day2['x'], day2['y']); time.sleep(1.0)
            val_2 = el.input_value()
            log(f"  [date] [1] After day 2 click: '{val_2}'")
            if val_2 and "jan 2025" not in val_2.lower():
                # Day 2 committed! Now click day 1.
                log(f"  [date] [1] Day 2 committed! Now clicking day 1...")
                reopen_to_sep()
                day1 = get_day_coord("1")
                if day1:
                    page.mouse.click(day1['x'], day1['y']); time.sleep(1.0)
                    val_1a = el.input_value()
                    log(f"  [date] [1] After day 1 (post-day2): '{val_1a}'")
                    if "sep" in val_1a.lower() and "2026" in val_1a:
                        log(f"  [date] [1] Two-step succeeded!"); esc(page)
                        ss(page, "v14_date_ok"); return True

        # ── Approach 2: Patchright tap() on day 1 (touch events) ───────────────
        # Touch events are completely separate from mouse events — React may handle them
        log(f"  [date] [2] Patchright tap() on day 1 cell...")
        reopen_to_sep()
        td_locator = page.locator('td[class*="ocpicker"]')
        cnt = td_locator.count()
        log(f"  [date] [2] {cnt} ocpicker TDs")
        exact_1 = None
        for i in range(min(cnt, 35)):
            try:
                t = td_locator.nth(i).inner_text(timeout=500).strip()
                if t == "1": exact_1 = td_locator.nth(i); break
            except: pass
        if exact_1:
            try:
                exact_1.scroll_into_view_if_needed()
                exact_1.tap(timeout=5000)   # tap() fires touchstart/touchend events
                time.sleep(1.0)
                val_tap = el.input_value()
                log(f"  [date] [2] After tap(): '{val_tap}'")
                if "sep" in val_tap.lower() and "2026" in val_tap:
                    log(f"  [date] [2] tap() succeeded!"); esc(page)
                    ss(page, "v14_date_tap_ok"); return True
            except Exception as ex:
                log(f"  [date] [2] tap failed: {str(ex)[:80]}")

        # ── Approach 3: React fiber — find and call the real onClick handler ───
        log(f"  [date] [3] React fiber direct call...")
        reopen_to_sep()
        result_fiber = page.evaluate("""() => {
            // Look at ALL ocpicker cells and find any that have a React onClick handler
            let cells = document.querySelectorAll('td[class*="ocpicker"]');
            let results = [];
            for (let td of cells) {
                let fiberKey = Object.keys(td).find(k =>
                    k.startsWith('__reactFiber') || k.startsWith('__reactInternalInstance'));
                if (!fiberKey) continue;
                let fiber = td[fiberKey];
                ...
                while (node && depth < 8) {
                    ...
                    if (props.onClick) {
                        ...
                        results.push({txt, nodeType: typeof node.type === 'string' ? node.type : (node.type && node.type.name)||'?',
                            depth, hasHandler: true});
                        if (txt === '1') {
                            // Found day 1 with onClick — call it
                            let r = td.getBoundingClientRect();
                            let cx = r.left + r.width/2, cy = r.top + r.height/2;
                            let ev = {
                                nativeEvent: {clientX:cx, clientY:cy},
                                target: td, currentTarget: td,
                                type:'click', bubbles:true,
                                preventDefault:()=>{}, stopPropagation:()=>{},
                                persist:()=>{}, isDefaultPrevented:()=>false,
                            };
                            try { props.onClick(ev); return 'fiber-day1-onClick: ok, depth='+depth; }
                            catch(ex) { return 'fiber-day1-err: '+ex.message; }
                        }
                        break;
                    }
                    node = node.return;
            ...
            return 'scanned ' + results.length + ' cells: ' + JSON.stringify(results.slice(0,5));
        }""")
        log(f"  [date] [3] Fiber scan: {result_fiber}")
        time.sleep(1.0)
        val_f = el.input_value()
        log(f"  [date] [3] After fiber: '{val_f}'")
        if "sep" in val_f.lower() and "2026" in val_f:
            log(f"  [date] [3] Fiber succeeded!"); esc(page)
            ss(page, "v14_date_fiber_ok"); return True

        # ── Approach 4: Dispatch on the PARENT of the cell (tr or table) ───────
        # Maybe the calendar's click handler is on a parent element, not the TD itself.
        # React often uses event delegation — one listener on a container.
        log(f"  [date] [4] Dispatch on calendar container...")
        reopen_to_sep()
        result_parent = page.evaluate("""() => {
            let cells = document.querySelectorAll('td[class*="ocpicker"]');
            let day1 = null;
            for (let td of cells) {
                if (td.textContent.trim() === '1') { day1 = td; break; }
            }
            if (!day1) return 'no-day1';
            let r = day1.getBoundingClientRect();
            let cx = r.left + r.width/2, cy = r.top + r.height/2;
            // Try clicking on ancestors: TR, TBODY, TABLE, and the calendar container div
            let candidates = [];
            let p = day1.parentElement;
            while (p && candidates.length < 6) {
                candidates.push(p);
                p = p.parentElement;
            }
            let dispatched = [];
            // Fire click on each ancestor, checking if the input value changes
            let inputEl = document.getElementById('Available_Start_Date_start_date');
            let before = inputEl ? inputEl.value : '?';
            for (let c of candidates) {
                ...
                if (inputEl && inputEl.value !== before) {
                    dispatched.push('SUCCESS on ' + c.tagName + '.' + c.className.substring(0,30));
                    break;
                }
                dispatched.push(c.tagName);
            }
            let after = inputEl ? inputEl.value : '?';
            return 'before='+before+' after='+after+' path='+dispatched.join('>');
        }""")
        log(f"  [date] [4] Parent dispatch: {result_parent}")
        time.sleep(1.0)
        val_p = el.input_value()
        log(f"  [date] [4] After parent dispatch: '{val_p}'")
        if "sep" in val_p.lower() and "2026" in val_p:
            log(f"  [date] [4] Parent dispatch succeeded!"); esc(page)
            ss(page, "v14_date_parent_ok"); return True

        # ── Approach 5: Use Patchright's page.mouse.click() on day 2, check, then day 1 ─
        # Try a fresh click sequence with explicit focus management
        log(f"  [date] [5] Fresh focus + day 15 click approach...")
        reopen_to_sep()
        # First bring the calendar into focus by clicking the header/nav area
        hdr_coord = page.evaluate("""() => {
            let all = document.querySelectorAll('*');
            for (let el of all) {
                if (el.children.length === 0 && /^sep$/i.test(el.textContent.trim())) {
        ...
        log(f"  [date] [5] Header 'Sep' coord: {hdr_coord}")
        if hdr_coord:
            page.mouse.move(hdr_coord['x'], hdr_coord['y']); time.sleep(0.1)

        day15 = get_day_coord("15")
        if day15:
            page.mouse.move(day15['x'], day15['y']); time.sleep(0.2)
            page.mouse.click(day15['x'], day15['y']); time.sleep(1.5)
            val_15b = el.input_value()
            log(f"  [date] [5] After day 15: '{val_15b}'")
            if val_15b and "jan 2025" not in val_15b.lower():
                reopen_to_sep()
                day1b = get_day_coord("1")
                if day1b:
                    page.mouse.move(day1b['x'], day1b['y']); time.sleep(0.2)
                    page.mouse.click(day1b['x'], day1b['y']); time.sleep(1.5)
                    val_5 = el.input_value()
                    log(f"  [date] [5] After day 1: '{val_5}'")
                    if "sep" in val_5.lower() and "2026" in val_5:
                        log(f"  [date] [5] Approach 5 succeeded!"); esc(page)
                        ss(page, "v14_date_app5_ok"); return True

        time.sleep(0.5)
        esc(page); time.sleep(0.5)
        ss(page, "v14_date_after_all")
        el.scroll_into_view_if_needed(); time.sleep(0.3)
        ss(page, "v14_date_field_view")
