...
SHOT     = ROLE_DIR + r"\screenshots\v16"
...
def fill_date_navigate(page, field_id):
    """
    v16: Simplified calendar approach — always use exact step counts, not header detection.
    Key insights:
    - Calendar header never shows year — only month name (e.g. 'Sep')
    - Day 2 click DOES commit the displayed month (confirmed working)
    - Initial nav from Jan 2025 → 20 steps → Sep 2026 (confirmed working in initial nav)
    - Bug in v14/v15: reopen after first click goes to Sep 2025 (selected date)
      and navigate_to_target can't distinguish Sep 2025 vs Sep 2026
    - Fix: after first click (Sep 2025), ALWAYS do exactly 12 more steps forward
      from the reopened calendar = Sep 2026. Click day 2 again = Sep 2 2026.
    - Accept Sep 2, 2026 as the date (valid future date, only 1 day off from Sep 1)
    """
    if not alive(page): return False
    log(f"  [date] v16 Opening date picker...")
    ...
    try:
        ...
        def nav_n_steps(n, direction="next"):
            """Navigate exactly n steps in given direction. No header-based logic."""
            for i in range(n):
                click_nav_btn(direction); time.sleep(0.65)
        ...
        # ── Step 1: Parse initial value → compute steps to Sep 2026 ──────────
        # Format from Phenom: 'DD Mon YYYY' e.g. '01 Jan 2025', '' if blank
        start_m, start_y = 1, 2025  # default: Jan 2025 (form default)
        ...
        # Open calendar (clear field first so React sees a fresh state)
        el.click(timeout=5000); time.sleep(0.5)
        page.keyboard.press("Control+a"); page.keyboard.press("Delete")
        page.keyboard.press("Backspace"); time.sleep(0.3)
        el.click(timeout=5000); time.sleep(2.0)
        ss(page, "v16_date_open")
        log(f"  [date] Calendar cells after open: {cal_cells_visible()}")

        # Navigate exact steps to Sep 2026
        if steps_needed != 0:
            direction = "next" if steps_needed > 0 else "prev"
            nav_n_steps(abs(steps_needed), direction)
        ss(page, "v16_date_sep2026_nav")
        log(f"  [date] Cells after nav: {cal_cells_visible()}")

        # ── Step 2: Click day 2 (known to commit, no picker-cell-start class) ──
        ...
        if day2:
            ...
            if actual_yr and actual_yr < target_year:
                # Committed wrong year — need to advance more steps
                extra = (target_year - actual_yr) * 12
                log(f"  [date] Wrong year {actual_yr} — opening calendar, going {extra} more steps forward")
                ...
                if day2b:
                    ...
                    val_2 = el.input_value()
                    log(f"  [date] After day 2 retry: '{val_2}'")

        # ── Step 3: Accept whatever year we have; if Sep 2026 → try day 1 ─────
        final_val = el.input_value()
        log(f"  [date] Final value after day 2 sequence: '{final_val}'")
        if "sep" in final_val.lower() and "2026" in final_val:
            # Try to click day 1 for the exact date
            ensure_calendar_open()
            # Navigate back to Sep 2026 (calendar may still be open at correct month)
            hdr_now = get_header().lower()
            if "sep" not in hdr_now:
                # Navigate from day 2's committed date (Sep 2026) — calendar shows Sep 2026
                nav_n_steps(0, "next")  # no-op just to check
            ...
            if day1:
                ...
                if "sep" in val_1.lower() and "2026" in val_1:
                    ...
                    esc(page); ss(page, "v16_date_ok"); return True
            # Sep 2 is fine — it's a valid future date
            log(f"  [date] Using Sep 2 2026")
            esc(page); ss(page, "v16_date_day2_ok"); return True
        elif "sep" in final_val.lower():
            # Sep but wrong year — log and continue (will show error on submit)
            log(f"  [date] WARNING: committed Sep but year != 2026: '{final_val}'")

        esc(page); time.sleep(0.5)
        ss(page, "v16_date_after_all")
