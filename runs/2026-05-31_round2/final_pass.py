...
env=dict(os.environ); env["CDP_PORT"]="9333"
# essay-approved (now have answers) + remaining near-misses
TARGETS=["duolingo_manager_strategy_and","duolingo_recruiter_ii_nyc","duolingo_recruiter_ii_pittsburgh",
 "discord_recruiting_data_analyst","samsara_enablement_business_partner","samsara_recruiting_coordinator",
 "cursor_recruiting_coordinator","cohere_recruitment_coordinator","crusoe_workplace_coordinator",
 "sierra_recruiting_coordinator_contract"]
results=[]
for name in TARGETS:
    d=DISC/name
    ...
    try:
        ...
        for ln in (r.stdout or "").splitlines():
            if any(k in ln for k in ["Success=","SUBMITTED","unanswered","ESSAY REVIEW","essay:approved"]): print("  "+ln,flush=True)
        st="?"
        if sj.exists():
    ...
    except subprocess.TimeoutExpired: results.append((name,"timeout"))
    except Exception as e: results.append((name,"err"))
print("\n=== FINAL PASS SUMMARY ===",flush=True)
