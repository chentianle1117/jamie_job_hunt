#!/usr/bin/env python3
"""Verify resume layout: margins, bottom whitespace, bullet wraps."""
import json, re
...
def build_resume_html(data):
    ...
    top = margins.get("top", "0.22in")
    right = margins.get("right", "0.40in")
    bottom = margins.get("bottom", "0.22in")
    left = margins.get("left", "0.40in")
...
--summary-margin:2pt 0;--section-header-margin:4pt 0 2pt 0;
--job-margin-bottom:5pt;--edu-margin-bottom:5pt;--edu-ul-margin-top:2pt;
--accent-color:#2a6496;}}
...
with sync_playwright() as p:
    ...
    # Check for any bullet wrapping (any li taller than 2x line height)
    wraps = page.evaluate("""
    () => {
        const lis = document.querySelectorAll('li');
        const wrapping = [];
        lis.forEach((li, i) => {
            if (li.offsetHeight > 26) {  // ~2 lines at 9.2pt*1.25
                wrapping.push({index: i, height: li.offsetHeight, text: li.textContent.slice(0, 80)});
            }
        });
        return wrapping;
    }
    """)
    # Take a cropped screenshot at the very bottom of the page to verify bottom whitespace
    page.set_viewport_size({"width": 816, "height": 1056})
    page.screenshot(path="/Users/jamiecheng/jamie_job_hunt/runs/2026-06-10_cambia/verify_bottom.png",
                    clip={"x": 0, "y": 950, "width": 816, "height": 106})
    print(f"Body height: {h}px (1056px max)")
    print(f"Overflow: {max(0, h-1056)}px")
    print(f"Bottom whitespace: ~{1056-h}px")
    if wraps:
        print(f"WRAPPING BULLETS ({len(wraps)}):")
        for w in wraps:
            print(f"  [{w['index']}] h={w['height']}px: {w['text']}")
    else:
        print("No wrapping bullets detected")
    browser.close()
