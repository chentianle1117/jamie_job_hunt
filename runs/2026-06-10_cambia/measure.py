#!/usr/bin/env python3
"""Measure actual page height with current settings."""
import json, re
...
def build_resume_html(data, margins):
...
margins_to_test = [
    ("0.30in","0.45in","0.30in","0.45in"),
    ("0.25in","0.40in","0.25in","0.40in"),
]
...
with sync_playwright() as p:
    ...
    for m in margins_to_test:
        html = build_resume_html(data, m)
        page.set_content(html, wait_until="networkidle")
        page.wait_for_timeout(200)
        h = page.evaluate("document.body.scrollHeight")
        print(f"Margins {m[0]}/{m[2]}: height={h}px (1056px = 1 page)")
    browser.close()
