...
JSON_NAME = "AdventureEXP_Jamie (Yi-Chieh) Cheng_Experiential-Education-Program-Ops_2026-06-10"
JSON_PATH = REPO / "tailored_resumes" / f"{JSON_NAME}.json"
PDF_PATH  = REPO / "tailored_resumes" / f"{JSON_NAME}.pdf"
RUN_DIR   = REPO / "runs" / "2026-06-10_adventureexp"
...
def build_resume_html(data, scale=1.0):
    meta = data.get("meta", {})
...
<style>
  :root {{
    --body-font-size: {9.5*scale:.2f}pt;
    --bullet-font-size: {9.2*scale:.2f}pt;
    --summary-font-size: {9*scale:.2f}pt;
    --header-name-size: 19pt;
    --header-contact-size: 8.8pt;
    --section-header-size: {9*scale:.2f}pt;
    --job-title-size: {9.5*scale:.2f}pt;
...
def main():
    ...
    # autoFit: shrink font scale toward the fontRange floor until it fits on 1 page.
    # fontRange [min,max] in the JSON; 9.5pt body == scale 1.0, floor = min/9.5.
    fr = data.get("meta", {}).get("fontRange", [8.5, 10.0])
    floor_scale = fr[0] / 9.5
    with sync_playwright() as p:
        ...
        scale = 1.0
        page_count = 99
        html = build_resume_html(data, scale)
        while True:
            ...
            if page_count == 1 or scale <= floor_scale + 1e-9:
                break
            scale = max(floor_scale, scale - 0.03)
            html = build_resume_html(data, scale)
        print(f"[PDF] Written to {PDF_PATH} (scale={scale:.2f}, body≈{9.5*scale:.1f}pt)", flush=True)
        print(f"[PAGES] {page_count}", flush=True)

        if page_count != 1:
            print(f"[BLOCKER] Resume is {page_count} pages even at min font {fr[0]}pt — trim content!", flush=True)
            browser.close()
            sys.exit(1)
